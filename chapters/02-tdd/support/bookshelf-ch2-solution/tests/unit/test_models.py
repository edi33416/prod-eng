...
def test_book_create_future_year_raises() -> None:
    with pytest.raises(ValidationError) as exc_info:
        BookCreate(title="X", author="Y", isbn="9780135957059", year=2099)
    assert "future" in str(exc_info.value).lower()


def test_review_create_valid() -> None:
    review = ReviewCreate(rating=5, text="Excellent!", reviewer="alice")
    assert review.rating == 5
    assert review.reviewer == "alice"


def test_review_create_rating_too_high_raises() -> None:
    with pytest.raises(ValidationError):
        ReviewCreate(rating=6, text="Too good!", reviewer="bob")


def test_review_create_rating_too_low_raises() -> None:
    with pytest.raises(ValidationError):
        ReviewCreate(rating=0, text="Terrible", reviewer="bob")
